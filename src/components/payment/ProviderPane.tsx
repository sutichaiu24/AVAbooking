...
export function ProviderPane({ method, providers, amount, onSettled }: Props) {
  ...
  return (
    <div className="p-5">
      ...
      <p className="mt-1 text-[15px] text-aa-muted">
      ...
      <div className={`mt-4 grid gap-2.5 ${isBanking ? "sm:grid-cols-2" : "sm:grid-cols-2"}`}>
        {providers.map((item) => {
          ...
          return (
            ...
            >
              <span
                className={`flex h-10 w-10 shrink-0 items-center justify-center text-[15px] font-medium text-white ${item.swatch}`}
                aria-hidden
              >
                {item.mark}
              </span>
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-[15px] text-aa-muted">{item.nameTh}</span>
              </span>
      ...
      {provider?.deepLink && (
        <p className="mt-3 flex items-center gap-1.5 font-mono text-[13px] text-aa-muted">
      ...
      {error && (
        <p role="alert" className="mt-3 bg-aa-red/10 px-3 py-2 text-[15px] font-semibold text-aa-crimson">
          {error}
        </p>
      )}
